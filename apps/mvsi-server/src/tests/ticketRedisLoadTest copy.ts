...
// Ensure search index exists (recreate if missing)
async function ensureSearchIndex(): Promise<boolean> {
  try {
    ...
    try {
      ...
      console.log("Redis Search index recreated successfully");
      return true;
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      console.error("Failed to recreate Redis Search index:", message);
      return false;
    }
  }
}
...
async function searchMatchingTickets(
  ...
  // Validate regions and build query before try block
  ...
  try {
    ...
    if (message.includes("No such index") || message.includes("idx:tickets")) {
      console.warn(`Index ${INDEX_NAME} not found, attempting to recreate...`);
      const indexRecreated = await ensureSearchIndex();
      if (indexRecreated) {
        console.log(`Index ${INDEX_NAME} recreated, retrying search...`);
        // Retry the search once
        try {
          const retryResult = await redisClient.ft.search(INDEX_NAME, query, {
            LIMIT: { from: 0, size: 10 },
          });
          
          if (retryResult?.documents && retryResult.documents.length > 0) {
            const currentTicketKey = MATCHMAKING_TICKET_KEY(ticket.matchmakingRequestId);
            const matchingTicketKeys = retryResult.documents
              .map((doc) => doc.id)
              .filter((key) => key !== currentTicketKey);
          ...
          console.error(`Retry search failed: ${retryMessage}`);
        }
      }
    }
